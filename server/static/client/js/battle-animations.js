...
var html=this.getSidebarHTML(side,'near');

var nearSides=this.battle.sides.filter(function(s){return!s.isFar&&s!==side;});for(var _i12=0;_i12<
nearSides.length;_i12++){var s=nearSides[_i12];
...
var html=this.getSidebarHTML(side,'far');
var farSides=this.battle.sides.filter(function(s){return s.isFar&&s!==side;});for(var _i14=0;_i14<
farSides.length;_i14++){var s=farSides[_i14];
...
if(!skipEmpty){for(var _i20=0,_this$$sprites2=
this.$sprites;_i20<_this$$sprites2.length;_i20++){var $spritesContainer=_this$$sprites2[_i20];
$spritesContainer.empty();
}
}
var numSides=this.battle.sides.length;
var isMassFFA=this.battle.gameType==='freeforall'&&numSides>4;for(var _i22=0,_this$battle$sides6=

this.battle.sides;_i22<_this$battle$sides6.length;_i22++){var _side$missedPokemon;var side=_this$battle$sides6[_i22];
if(isMassFFA){

var angle=2*Math.PI*side.n/numSides-Math.PI/2;
...
var angle=2*Math.PI*i/numSides-Math.PI/2;
var cx=fieldWidth/2+Math.cos(angle)*(fieldWidth*0.35);
var cy=fieldHeight/2+Math.sin(angle)*(fieldHeight*0.3);
var w=Math.max(60,120-numSides*2);
var h=Math.max(60,140-numSides*2);
...
var newBGNum=0;
for(var siden=0;siden<this.battle.sides.length;siden++){
var side=this.battle.sides[siden];

var spriteIndex=this.battle.sides.length>4?
side.isFar?1:0:
...
var terrain='';for(var _i26=0,_this$battle$pseudoWe4=
...
this.battle.sides;_i28<_this$battle$sides8.length;_i28++){var side=_this$battle$sides8[_i28];
...
this.scene.battle.sides;_i32<_this$scene$battle$si2.length;_i32++){var side=_this$scene$battle$si2[_i32];for(var _i34=0,_side$active4=
side.active;_i34<_side$active4.length;_i34++){var active=_side$active4[_i34];
...
types.length;_i38++){var type=types[_i38];
...
for(var _i40 in pokemon.movestatuses){
status+=PokemonSprite.getEffectTag(_i40);
}
