...
var limit=this.gameType==='multi'?2:this.sides.length;
for(var i=0;i<limit;i++){
var side=this.sides[i];
if(!side)continue;for(var _i16=0,_side$active2=
...
this.nearSide=side;
this.farSide=this.sides.find(function(s){return s!==side;})||this.sides[0];for(var _i25=0,_this$sides4=
this.sides;_i25<_this$sides4.length;_i25++){var s=_this$sides4[_i25];
s.isFar=s!==side;
}
}else{
...
for(var i=2;i<this.sides.length;i++){
...
var sides=numSides===4?
[this.sides[0],this.sides[3],this.sides[1],this.sides[2]]:
this.sides.slice();
var temp={};for(var _i27=0;_i27<
sides.length;_i27++){var side=sides[_i27];
temp[side.n]={};
...
var targetSide=sides[(i+1)%sides.length];
...
var side2=this.sides[1];for(var _i29=0;_i29<
sideConditions.length;_i29++){var _id2=sideConditions[_i29];
...
updateTurnCounters=function updateTurnCounters(){for(var _i31=0,_this$pseudoWeather4=
this.pseudoWeather;_i31<_this$pseudoWeather4.length;_i31++){var pWeather=_this$pseudoWeather4[_i31];
if(pWeather[1])pWeather[1]--;
if(pWeather[2])pWeather[2]--;
}for(var _i33=0,_this$sides6=
this.sides;_i33<_this$sides6.length;_i33++){var side=_this$sides6[_i33];
for(var id in side.sideConditions){
var cond=side.sideConditions[id];
if(cond[2])cond[2]--;
if(cond[3])cond[3]--;
}
}for(var _i35=0,_this$sides8=
this.sides;_i35<_this$sides8.length;_i35++){var _side=_this$sides8[_i35];for(var _i37=0,_side$active4=
_side.active;_i37<_side$active4.length;_i37++){var poke=_side$active4[_i37];
if(poke){
...
}else if(['all','allAdjacent','allAdjacentFoes','foeSide'].includes(moveTarget)){for(var _i39=0,_this$getAllActive6=
this.getAllActive();_i39<_this$getAllActive6.length;_i39++){var active=_this$getAllActive6[_i39];
...
getPressurePP=function getPressurePP(pp,foes){for(var _i41=0;_i41<
foes.length;_i41++){var foe=foes[_i41];
...
}else{for(var _i43=0,_kwArgs$spread$split2=
kwArgs.spread.split(',');_i43<_kwArgs$spread$split2.length;_i43++){var hitTarget=_kwArgs$spread$split2[_i43];
...
switch(_effect.id){
case'lunardance':for(var _i45=0,_poke$moveTrack2=
_poke.moveTrack;_i45<_poke$moveTrack2.length;_i45++){var trackedMove=_poke$moveTrack2[_i45];
trackedMove[1]=0;
}
...
var stats=args[3]?args[3].split(', '):['atk','def','spa','spd','spe','accuracy','evasion'];for(var _i47=0;_i47<
stats.length;_i47++){var _stat3=stats[_i47];
...
var _stats=args[3]?args[3].split(', '):['atk','def','spa','spd','spe','accuracy','evasion'];for(var _i49=0;_i49<
_stats.length;_i49++){var _stat6=_stats[_i49];
_poke8.boosts[_stat6]=frompoke.boosts[_stat6];
if(!_poke8.boosts[_stat6])delete _poke8.boosts[_stat6];
}
if(this.gen>=6){
var volatilesToCopy=['focusenergy','gmaxchistrike','laserfocus'];for(var _i51=0;_i51<
volatilesToCopy.length;_i51++){var volatile=volatilesToCopy[_i51];
...
case'-clearallboost':{
var timeOffset=this.scene.timeOffset;for(var _i53=0,_this$getAllActive8=
this.getAllActive();_i53<_this$getAllActive8.length;_i53++){var active=_this$getAllActive8[_i53];
active.boosts={};
...
case'-cureteam':{
var _poke22=this.getPokemon(args[1]);for(var _i55=0,_poke22$side$pokemon2=
_poke22.side.pokemon;_i55<_poke22$side$pokemon2.length;_i55++){var _target2=_poke22$side$pokemon2[_i55];
_target2.status='';
...
_poke28.addVolatile('formechange',speciesForme);for(var _i57=0,_tpoke$moveTrack2=
tpoke.moveTrack;_i57<_tpoke$moveTrack2.length;_i57++){var _trackedMove=_tpoke$moveTrack2[_i57];
...
_poke36.removeTurnstatus('protect');for(var _i59=0,_poke36$side$pokemon2=
_poke36.side.pokemon;_i59<_poke36$side$pokemon2.length;_i59++){var curTarget=_poke36$side$pokemon2[_i59];
...
var foeActive=[];for(var _i61=0,_poke36$side$foe$acti2=
_poke36.side.foe.active;_i61<_poke36$side$foe$acti2.length;_i61++){var maybeTarget=_poke36$side$foe$acti2[_i61];
...
_side2.addSideCondition(_effect20,!!kwArgs.persistent);
...
_side3.removeSideCondition(_effect21.name);
this.log(args,kwArgs);
break;
}
...
switch(_effect23.id){
case'gravity':
if(this.seeking!==null)break;for(var _i63=0,_this$getAllActive10=
this.getAllActive();_i63<_this$getAllActive10.length;_i63++){var _active=_this$getAllActive10[_i63];
...
var sideMatch=/^p(\d+)($|: )/.exec(name);
var slotMatch=/^p(\d+)([a-f]): /.exec(name);
if(slotMatch){
...
var searchid=serverPokemon.ident+"|"+serverPokemon.details;for(var _i65=0,_this$sides$siden$pok2=
this.sides[siden].pokemon;_i65<_this$sides$siden$pok2.length;_i65++){var pokemon=_this$sides$siden$pok2[_i65];
...
if(!isInactive&&side.active[slot])return side.active[slot];for(var _i67=0,_side$pokemon2=

side.pokemon;_i67<_side$pokemon2.length;_i67++){var pokemon=_side$pokemon2[_i67];
...
var pMatch=/^p(\d+)($|:)/.exec(sidename);
if(pMatch){
var n=parseInt(pMatch[1],10)-1;
...
this.updateTurnCounters();for(var _i69=0,_this$sides10=

this.sides;_i69<_this$sides10.length;_i69++){var side=_this$sides10[_i69];
...
}else{for(var _i71=0,_this$sides12=

this.sides;_i71<_this$sides12.length;_i71++){var _side4=_this$sides12[_i71];
_side4.active=[null];
}
}
break;
...
break;
default:for(var _i73=0,_this$sides14=
this.sides;_i73<_this$sides14.length;_i73++){var _side5=_this$sides14[_i73];_side5.active=[null];}
break;
}
...
var playerMatch=/^p(\d+)$/.exec(args[1]);
if(playerMatch){
var n=parseInt(playerMatch[1],10)-1;
if(n>=this.sides.length&&this.gameType==='freeforall'){

for(var i=this.sides.length;i<=n;i++){
var newSide=new Side(this,i);


newSide.foe=this.sides[0];
newSide.isFar=newSide!==this.mySide;
newSide.active=[null];
...
_side6.setName(args[2]);
if(args[3])_side6.setAvatar(args[3]);
if(args[4])_side6.rating=args[4];
...
if(!_side7.badges.includes(badge))_side7.badges.push(badge);
...
case'clearpoke':{for(var _i75=0,_this$sides16=
this.sides;_i75<_this$sides16.length;_i75++){var _side9=_this$sides16[_i75];
_side9.clearPokemon();
}
break;
}
...
var _side10=this.sides[siden];
for(var _i76=0;_i76<_side10.pokemon.length;_i76++){
var _pokemon2=_side10.pokemon[_i76];
if(_pokemon2.details!==args[2]&&_pokemon2.checkDetails(args[2])){
_side10.addPokemon('','',args[2],_i76);
break;
}
}
break;
...
var _side11=this.getSide(args[1]);
_side11.openTeamSheet=true;
_side11.clearPokemon();for(var _i78=0;_i78<
team.length;_i78++){var set=team[_i78];
var details=set.species+(!set.level||set.level===100?'':", L"+set.level)+(
!set.gender||set.gender==='N'?'':", "+set.gender)+(set.shiny?', shiny':'');
var _pokemon3=_side11.addPokemon('','',details);
if(set.item)_pokemon3.item=set.item;
if(set.ability)_pokemon3.rememberAbility(set.ability);for(var _i80=0,_set$moves2=
set.moves;_i80<_set$moves2.length;_i80++){var move=_set$moves2[_i80];
...
stack.length;_i82++){var line=stack[_i82];
if(/\brun\b/.test(line)){
break;
}
this.log(['error',line]);
